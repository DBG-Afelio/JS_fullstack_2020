...
const stringOrNumber: [number[], string[]] = [sequence, animals];
const stringOrNumberArray: (number[] | string[])[] = [sequence, animals];

// Ecrire une fonction qui prend un paramètre nom obligatoire et un prenom facultatif
// et renvoie le "nom complet"

function nomcompplet(nom: string, prenom?: string): string {
    return nom + prenom;
}
...
function salut(salutation = "SALUT"):string{
...
function getPointsFor(letter:string):number{
  ...
  return lettersAndPoints.reduce((computeScore, pointsTuple) => {
