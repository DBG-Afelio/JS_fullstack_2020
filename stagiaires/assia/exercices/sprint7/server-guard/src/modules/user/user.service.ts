...
export class UserService {
    ...
    async getAll(): Promise<UsersEntity[]> {
        const list: UsersEntity[] = await this.usersRepo.find()
        .catch((error) => {
    ...
    async findByLogin(login: string): Promise<UsersEntity> {
        ...
        const found = all.find((userScreened: UsersEntity) => userScreened.login === login);
        if (!found){
      ...
      async update(id: number, userUpdated: UpdateUserDto): Promise<UsersEntity> {
        ...
        if(user) {
          ...
          let update: DeepPartial<UsersEntity> = await this.usersRepo.preload(userUpdated);
  //      update.roles = userUpdated.roles;
        if(userUpdated.role){
            update = {...update, role: userUpdated.role};
        }
          return await this.usersRepo.save(update)
          .catch((error) => {
            throw new BadRequestException(error.message, error.name);
          });
        }
