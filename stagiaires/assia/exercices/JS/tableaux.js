...
function matricePositive(arr2d) {
    return !(arr2d.some(arr => arr.some(el => el < 0)));
};
...
function listeNoms(arrIn) {
    let arr = arrIn;
    const lastWord = arr.pop();
    return arr.join(',').concat(' et ', lastWord);
}
...
function initiales(nameStr) {
    return nameStr.trim().split(' ').map(name => name.charAt(0).toUpperCase()).join('.') + '.';
}
...
tests.push(['concateneTableaux', concateneTableaux([1, 2, 7, 8], [2, 4, 5, 6]) === [1, 2, 7, 8, 2, 4, 5, 6]]);  
...
function doubleMatrice(arr2d) {
    return arr2d.map(arr => arr.map(nb => nb * 2));
}
...
function sommePositifsNegatifs(arr) {
    return arr.reduce((acc, curVal) => curVal >= 0 ? [acc[0] + curVal, acc[1]] : [acc[0], acc[1] + curVal], [0, 0]);
}
