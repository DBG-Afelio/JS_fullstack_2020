    ...
    return forkJoin(
      ...
      .pipe(
        map(([arrayArticleDto , arrayAuteur]: [ ArticleDto[], Auteur[] ]) => {
          return arrayArticleDto.map(articleDto => Article.fromDto(articleDto, arrayAuteur.find(auteur => { 
            console.log(articleDto.auteur_id, auteur.id);
            return articleDto.auteur_id == auteur.id ;
          })));
        }),
      )
    ;
