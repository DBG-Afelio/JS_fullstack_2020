...
function generateMaree (tabColors, lines, column, divParent){
    for (let i = 1; i < lines+1; i++) {
        for (let j = 1; j < column+1; j++) {
            ...
            divParent.appendChild(carre);
            carre.classList.add("carre");
            const carreLine=i.toString();
            const carreColumn=j.toString();
            carre.setAttribute('data-ligne',carreLine);
            carre.setAttribute('data-colonne',carreColumn);
            carre.setAttribute('data-color',tabColors[randomNumber(0,tabColors.length)]);
            carre.style.gridColumn=carreColumn;
            carre.style.gridRow=carreLine;
...
function generateButtons(tabColors){
    for (let i = 0; i < tabColors.length; i++) {
        ...
        button.setAttribute('data-color',tabColors[i]);
        button.addEventListener('click',(event)=>play(event.target.getAttribute('data-color')));
    }
...
function changeColor(oldColor, newColor, div){
    const tabDiv=[getHaut(div),getBas(div),getDroite(div),getGauche(div)];
...
function getHaut(div) {
    const ligne=parseInt(div.getAttribute('data-ligne'),10);
    const colonne=parseInt(div.getAttribute('data-colonne'),10);
    return getDiv(ligne-1,colonne);
}
...
generateMaree(colors,5,5,maree);
