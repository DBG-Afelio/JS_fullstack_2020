...
export class TimerService {
  ...
  constructor() {
    this.tick$ = interval(100).pipe(tap((time) => console.log('timer:', time)));
  }
  ...
  public getTimer(dateStart: Date = new Date()): Observable<Time> {
    return this.tick$.pipe(
      map(() => this.getMillseconds(dateStart, new Date())),
      map((n) => this.getTimeFromMilliseconds(n))
    );
  }
  ...
  public isMultiWindowTimerStarted(id: string): Observable<Date | null> {
    ...
    return of(startDate).pipe(
      tap(() => console.log('storage state', startDate))
    );
  }
