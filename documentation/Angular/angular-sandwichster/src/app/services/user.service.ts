...
export class UserService {
  ...
  createUser(user: UserModel): Observable<UserModel> {
    return this.http
      .post<UserModel>(`http://localhost:3000/utilisateurs`, user)
      .pipe(
  ...
  updateUser(user: UserModel): Observable<UserModel> {
    return this.http
      .put<UserModel>(`http://localhost:3000/utilisateurs/${user.id}`, user)
      .pipe(catchError((error: any) => Observable.throw(error.json())));
  }

  removeUser(user: UserModel): Observable<UserModel> {
    return this.http
      .delete<any>(`http://localhost:3000/utilisateurs/${user.id}`)
      .pipe(catchError((error: any) => Observable.throw(error.json())));
  }
